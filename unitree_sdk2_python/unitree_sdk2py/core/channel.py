    ...
    def Init(self, id: int, networkInterface: str = None, qos: Qos = None):
        ...
        with self.__class__.__init_lock:
            ...
            if networkInterface is None:
                config = ChannelConfigAutoDetermine # 如果没有指定网卡，就让其自己判断使用哪个网卡
            else:
                config = ChannelConfigHasInterface.replace('$__IF_NAME__$', networkInterface) # 如果指定了网卡，就把ChannelConfigHasInterface里的'$__IF_NAME__$' 替换为指定的网卡的名字

            try:
                self.__class__.__domain = Domain(id, config) # 创建一个 DDS 通信域，并应用刚才的网卡配置。
            except DDSException as e:
            ...
            try:
                self.__class__.__participant = DomainParticipant(id) # 节点
            except DDSException as e:
