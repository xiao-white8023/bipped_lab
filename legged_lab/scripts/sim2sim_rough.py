...
def prepare_mjcf_for_custom_scene(model_path: str) -> tuple[str, tempfile.TemporaryDirectory | None]:
    """Return a MuJoCo XML path with included default floors removed for custom scenes."""
    ...
    if not model_needs_sanitizing and not strip_included_floor:
        return model_path, None

    temp_dir = tempfile.TemporaryDirectory(prefix="g1_rough_mjcf_")
    ...
    print(f"[INFO] Using temporary MuJoCo scene: {patched_scene_path}")
...
class G1RoughMujocoRunner:
    ...
    def update_command_from_joystick(self) -> bool:
        ...
        self.command_vel[0] = np.clip(-ax1 * 0.7, *self.cfg.command.lin_vel_x)
...
def main() -> None:
    ...
    try:
        runner = G1RoughMujocoRunner(cfg=cfg, policy_path=args.policy, model_path=load_model_path)
