...
from legged_lab.terrains import flex_terrain_CFG,GRAVEL_TERRAINS_CFG, ROUGH_TERRAINS_CFG  # noqa:F401
...
@configclass
class Reward:
    track_lin_vel_xy_exp = RewTerm(func=mdp.track_lin_vel_xy_yaw_frame_exp, weight=2, params={"std": 0.5})
    track_ang_vel_z_exp = RewTerm(func=mdp.track_ang_vel_z_world_exp, weight=2, params={"std": 0.5})
    ...
    body_orientation_l2 = RewTerm(
        func=mdp.body_orientation_l2, params={"asset_cfg": SceneEntityCfg("robot", body_names=".*torso.*")}, weight=-2
    )
...
class G129_ROUGHENVCFG:
    ...
    commands: CommandsCfg = CommandsCfg(
        ...
        ranges=CommandRangesCfg(
            lin_vel_x=(-0.6, 1.0), lin_vel_y=(-0.5,-0.5), ang_vel_z=(-1, 1), heading=(-math.pi, math.pi)
        ),
    )
