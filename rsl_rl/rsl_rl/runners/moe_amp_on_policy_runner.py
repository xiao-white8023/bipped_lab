...
import rsl_rl
from rsl_rl.algorithms import AMPPPO,MoeAmpPpO
...
class MoeAmpOnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        if self.alg_cfg["class_name"] in ["PPO", "AMPPPO","MoeAmpPpO"]:
        ...
        policy: MoeActorCritic | ActorCritic | ActorCriticRecurrent | StudentTeacher | StudentTeacherRecurrent = policy_class(
        ...
        print(f"DEBUG: Motion file observation dim: {amp_data.observation_dim}")
        obs, _ = self.env.get_observations() # 获取机器人实际观测
        amp_obs_demo = self.env.get_amp_obs_for_expert_trans()
        print(f"DEBUG: Real robot AMP observation dim: {amp_obs_demo.shape[1]}")
        ...
        self.alg: AMPPPO | MoeAmpPpO = alg_class(
            policy,
            discriminator,
            amp_data,
            amp_normalizer,
