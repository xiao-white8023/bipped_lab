...
import rsl_rl
from rsl_rl.algorithms import PPO, MoePPO
...
class FilmOnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        if self.alg_cfg["class_name"] not in ["PPO", "MoePPO"]:
        ...
        policy: ActorCritic | MoeActorCritic | MhaActorCritic = policy_class(
            ...
            with torch.inference_mode():
                    ...
                    if self.log_dir is not None:
                        if "episode" in infos:
                            ...
                            ep_infos.append(
                                self._copy_log_info(
                                    infos["log"],
                                    exclude_prefixes=("Curriculum/depth_noise_",),
                                )
                            )
                        # Update rewards
                        if self.alg.rnd:
                ...
                if hasattr(self.env, "update_depth_noise_curriculum_once"):
                    ...
                    if self.log_dir is not None and hasattr(self.env, "get_depth_noise_curriculum_log"):
                        ...
                        if depth_noise_log:
                            ep_infos.append(self._copy_log_info(depth_noise_log))
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        ep_string = ""
        if locs["ep_infos"]:
            for key in self._collect_ep_info_keys(locs["ep_infos"]):
                info_tensors = []
                for ep_info in locs["ep_infos"]:
                    # handle scalar and zero dimensional tensor infos
                    if key not in ep_info:
                        continue
                    infotensor = self._as_log_tensor(ep_info[key])
                    if infotensor is None or infotensor.numel() == 0:
                        continue
                    info_tensors.append(infotensor)
                if not info_tensors:
                    continue
    ...
    def load(self, path: str, load_optimizer: bool = True):
        ...
        if resumed_training:
            ...
            if hasattr(self.alg, "_set_policy_moe_iteration"):
                self.alg._set_policy_moe_iteration()
    ...
    def _copy_log_info(self, info: dict, exclude_prefixes: tuple[str, ...] = ()) -> dict:
        copied_info = {}
        for key, value in info.items():
            if any(key.startswith(prefix) for prefix in exclude_prefixes):
                continue
            if isinstance(value, torch.Tensor):
                copied_info[key] = value.detach().clone()
            else:
                copied_info[key] = value
        return copied_info

    @staticmethod
    def _collect_ep_info_keys(ep_infos: list[dict]) -> list:
        keys = []
        seen = set()
    ...
    def _as_log_tensor(self, value) -> torch.Tensor | None:
        if isinstance(value, torch.Tensor):
